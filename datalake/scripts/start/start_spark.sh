...
DRIVER_LOG_DFS_DIR=$(cat "${SPARK_HOME}"/conf/spark-defaults.conf | grep -i 'spark.driver.log.dfsDir' | tr -s ' ' | cut -d ' ' -f 2)

mkdir -p "${SPARK_LOG_DIR}" "${EVENT_LOG_DIR}" "${DRIVER_LOG_DFS_DIR}"
